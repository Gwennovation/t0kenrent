...
export default function WalletSelector({ onAuthenticated, compact = false }: WalletSelectorProps) {
  ...
  const [handcashConnecting, setHandcashConnecting] = useState(false)

  // HandCash OAuth configuration
  const HANDCASH_APP_ID = process.env.NEXT_PUBLIC_HANDCASH_APP_ID || 'your-handcash-app-id'
  const HANDCASH_REDIRECT_URI = typeof window !== 'undefined' 
    ? `${window.location.origin}/api/auth/handcash/callback`
    : ''
  ...
  async function connectHandCash() {
    ...
    setLoading(true)
    setError('')
    setHandcashConnecting(true)
    
    try {
      // HandCash Connect OAuth flow
      const handcashAuthUrl = `https://app.handcash.io/api/oauth/authorize?appId=${HANDCASH_APP_ID}&redirectUrl=${encodeURIComponent(HANDCASH_REDIRECT_URI)}&permissions=PAYMENT,USER_PUBLIC_PROFILE`
      
      // For demo purposes, we'll simulate the connection
      // In production, redirect to HandCash
      setTimeout(() => {
        // Simulate successful HandCash connection for demo
        const demoHandle = 'demo_user'
        const pseudoPublicKey = `hc_${demoHandle}_${Date.now()}`
        sessionStorage.setItem('handcash_handle', demoHandle)
        onAuthenticated(pseudoPublicKey, 'handcash')
        setLoading(false)
        setHandcashConnecting(false)
      }, 1500)
  ...
  async function submitPaymail() {
    ...
    try {
      // Generate a pseudo public key from paymail for tracking
      const pseudoPublicKey = `pm_${paymail.replace('@', '_at_')}_${Date.now()}`
      
      // Store paymail for payment references
      sessionStorage.setItem('user_paymail', paymail)
      
      // Simulate connection (in production, you would verify the paymail)
      setTimeout(() => {
    ...
    return (
      <div className="flex items-center gap-2">
        ...
        <button
          ...
          disabled={loading}
          className="p-2 rounded-lg bg-blue-500/10 hover:bg-blue-500/20 border border-blue-500/30 transition-all"
        ...
        <button
          ...
          disabled={loading}
          className="p-2 rounded-lg bg-purple-500/10 hover:bg-purple-500/20 border border-purple-500/30 transition-all"
          title="Use Paymail"
        >
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-1 gap-3">
        ...
        >
          ...
          <div className={`w-12 h-12 rounded-xl flex items-center justify-center transition-all ${
            ...
            <svg className={`w-7 h-7 transition-colors ${
              selectedWallet === 'handcash' ? 'text-white' : 'text-emerald-600 dark:text-emerald-400 group-hover:text-white'
          ...
          {handcashConnecting && selectedWallet === 'handcash' && (
        ...
        >
          ...
          <div className={`w-12 h-12 rounded-xl flex items-center justify-center transition-all ${
            ...
            <svg className={`w-7 h-7 transition-colors ${
              selectedWallet === 'metanet' ? 'text-white' : 'text-blue-600 dark:text-blue-400 group-hover:text-white'
            }`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M21 12a9 9 0 01-9 9m9-9a9 9 0 00-9-9m9 9H3m9 9a9 9 0 01-9-9m9 9c1.657 0 3-4.03 3-9s-1.343-9-3-9m0 18c-1.657 0-3-4.03-3-9s1.343-9 3-9m-9 9a9 9 0 019-9" />
            </svg>
          </div>
          ...
          <div>
            ...
            <div className="flex gap-2">
              ...
              <button
                ...
                className="px-6 py-3 bg-purple-500 hover:bg-purple-600 text-white font-medium rounded-lg transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
              >
                {loading ? (
      ...
      <p className="text-xs text-center text-surface-500 dark:text-surface-400">
        Choose a wallet to connect and start using T0kenRent
      </p>
    </div>
  )
}
